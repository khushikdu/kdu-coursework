...
function addPost() {
  ...
  if (postText.trim() !== "") {
    ...
    sendPostToApi({content : postText});
    console.log("...");
...
async function fetchAndAppendPosts() {
  if (loading || maxLoadCount === 0) 
    return; 
  loading = true;

  try {
    const response = await fetch(`http://localhost:8080/api/posts?pageNumber=${pageNumber}&pageSize=${pageSize}`);
    if (response.ok) {
...
function addPaginatedPost(post) {
  let postText = post
